// source: https://github.com/aurelia/path/blob/master/src/index.ts
// @ts-nocheck
...
let encodeKey = (k: number | string | boolean) => encode(k).replace("%24", "$");
  ...
  for (let j = 0; j <= keysLastIndex; j++) {
    ...
    if (j < keysLastIndex) {
      ...
      currentParams = currentParams[key] =
        prevValue || (isNaN(keys[j + 1] as number) ? {} : []);
    } else {
